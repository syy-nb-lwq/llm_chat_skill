...
> 注：本项目为单人单用户场景，已移除多租户（tenant_id），仅保留 owner token 身份认证与会话隔离。以下为历史评审原文。
...
召回必须带作用域过滤，最少包含 `user_id`（单用户场景，已移除 tenant_id）。项目记忆还应包含 `project_id`。检索结果应按相关性、时效性、置信度和重要度共同排序，并限制注入 token 数量。
...
> 注：本项目为单人单用户场景，已移除多租户（tenant_id），仅保留 owner token 防止未授权操作。
