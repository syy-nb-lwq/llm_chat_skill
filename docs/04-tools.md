...
│                                                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│  │ Python 源   │  │  MCP 源     │  │ HTTP 源     │            │
│  │ (builtin)   │  │ (外部服务)  │  │ (REST API) │            │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘            │
│         │                │                │                     │
│         └────────────────┼────────────────┘                     │
│                          ▼                                      │
...
| 组件 | 文件 | 说明 |
|------|------|------|
| ToolHub | `tools/hub.py` | 工具中枢,统一管理多源工具 |
| ToolSource | `tools/sources/base.py` | 工具源抽象基类 |
| PythonSource | `tools/sources/python_source.py` | Python 代码工具源 |
| MCPSource | `tools/sources/mcp_source.py` | MCP 协议工具源 |

### 11.3 工具源类型

| 类型 | 说明 | 配置项 |
|------|------|--------|
| `python` | Python 代码编写的工具 | `directories: List[str]` |
| `mcp` | MCP (Model Context Protocol) 工具 | `type: stdio\|http`, `server_path\|server_url` |
| `http` | HTTP REST API 工具 | `server_url` |
| `plugin` | 插件形式工具 | 自定义配置 |
...
# 注册 MCP 工具源
mcp_source = create_mcp_source(
    name="my_mcp",
    connection_type="stdio",
    server_path="/path/to/mcp_server",
)
hub.register_source(mcp_source)

# 连接所有源
await hub.connect_all()

# 调用工具(自动路由到对应源)
result = await hub.call_tool("weather_query", {"city": "北京"})
```
...
| 方法 | 说明 |
|------|------|
| `hub.register_source(source)` | 注册工具源 |
| `hub.connect_source(name)` | 连接指定源 |
| `hub.connect_all()` | 连接所有源 |
| `hub.call_tool(name, params)` | 调用工具 |
| `hub.list_tools()` | 列出所有工具 |
| `hub.schemas()` | 获取所有工具 schema |
| `hub.get_source_status()` | 获取源状态 |
...
- [x] 新建 `tools/registry.py` → 升级为 `tools/hub.py`
- [x] `ToolResult` 增 `meta` 字段,旧调用兼容(默认 `None` 的 meta 取空 dict)
- [x] `Tool` 加 `schema()` 抽象方法,旧工具实现该方法
- [x] `LearningAgent` 改持 `ToolHub`
- [x] 实现 `execute_dag()` + 参数变量解析
- [x] 加 `jsonschema`(参数校验)+ `pydantic`(schema 定义)
- [x] weather / search 工具按新接口改造
- [x] 写工具测试
- [x] 实现 ToolHub 多源管理